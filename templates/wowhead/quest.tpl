				...
				<table class="infobox">
					...
					<tr><td>
						...
						<ul>
							...
							{/if}{/strip}
							{if isset($quest.start)}<li><div><img src="/images/icons/small/quest_start.gif">{#Start#}: {section loop=$quest.start name=i}{if $quest.start[i].side}<span class="{$quest.start[i].side}-icon">{/if}<a href="/{$quest.start[i].type}-{$quest.start[i].entry}.html"{if $quest.start[i].type == 'item'} class="icontiny q{$quest.start[i].quality}" style="background-image: url(images/icons/tiny/{$quest.start[i].iconname}.gif);"{/if}>{$quest.start[i].name}</a>{if $quest.start[i].side}</span>{/if}{if $smarty.section.i.last}{else}, <br><span style="visibility: hidden;">{#Start#}: </span>{/if}{/section}</div></li>{/if}
							{if isset($quest.end)}<li><div><img src="/images/icons/small/quest_end.gif">{#End#}: {section loop=$quest.end name=i}{if $quest.end[i].side}<span class="{$quest.start[i].side}-icon">{/if}<a href="/{$quest.end[i].type}-{$quest.end[i].entry}.html">{$quest.end[i].name}</a>{if $quest.end[i].side}</span>{/if}{if $smarty.section.i.last}{else}, <br><span style="visibility: hidden;">{#End#}: </span>{/if}{/section}</div></li>{/if}
							...
							{section name=i loop=$quest.series}
								<tr>
									...
									<td>
										...
										{else}
											<div><a href="/quest-{$quest.series[i].entry}.html">{$quest.series[i].Title}</a></div>
										{/if}
									</td>
								</tr>
							{/section}
...
{strip}		{* Требует выполнения всех этих квестов *}
					...
					<tr><td>
							...
							{section name=i loop=$quest.req}
								<li>
									<div><a href="/quest-{$quest.req[i].entry}.html">{$quest.req[i].Title}</a></div>
								</li>
							{/section}
							</ul>
					</td></tr>
...
{strip}		{* Требует выполнения одного из этих квестов *}
					...
					<tr><td>
							...
							{section name=i loop=$quest.reqone}
								<li>
									<div><a href="/quest-{$quest.reqone[i].entry}.html">{$quest.reqone[i].Title}</a></div>
								</li>
							{/section}
							</ul>
					</td></tr>
...
{strip}		{* Требует активного квеста *}
					...
					<tr><td>
							...
							{section name=i loop=$quest.enabledby}
								<li>
									<div><a href="/quest-{$quest.enabledby[i].entry}.html">{$quest.enabledby[i].Title}</a></div>
								</li>
							{/section}
							</ul>
					</td></tr>
...
{strip}		{* Закрывает квесты *}
					...
					<tr><td>
							...
							{section name=i loop=$quest.closes}
								<li>
									<div><a href="/quest-{$quest.closes[i].entry}.html">{$quest.closes[i].Title}</a></div>
								</li>
							{/section}
							</ul>
					</td></tr>
...
{strip}		{* Во время выполнения квеста доступны эти квесты *}
					...
					<tr><td>
							...
							{section name=i loop=$quest.enables}
								<li>
									<div><a href="/quest-{$quest.enables[i].entry}.html">{$quest.enables[i].Title}</a></div>
								</li>
							{/section}
							</ul>
					</td></tr>
					...
					<tr><td>
							...
							{section name=i loop=$quest.open}
								<li>
										...
										{else}
											<div><a href="/quest-{$quest.open[i].entry}.html">{$quest.open[i].Title}</a></div>
										{/if}
								</li>
							{/section}
							</ul>
...
{/if}
				...
				<div class="text">
					...
					{if isset($quest.RequiredMinRep) or isset($quest.RequiredMaxRep)}
						...
						{strip}
							<ul>
							{if isset($quest.RequiredMinRep)}<li><div>{#Your_reputation_with#} <a href="/faction-{$quest.RequiredMinRep.entry}.html">{$quest.RequiredMinRep.name}</a> {#must_be#} {#higher_than#} {$quest.RequiredMinRep.value}</div></li>{/if}
							{if isset($quest.RequiredMaxRep)}<li><div>{#Your_reputation_with#} <a href="/faction-{$quest.RequiredMaxRep.entry}.html">{$quest.RequiredMaxRep.name}</a> {#must_be#} {#lower_than#} {$quest.RequiredMaxRep.value}</div></li>{/if}
							</ul>
							<div class="line"></div>
						{/strip}
					{/if}
	...
	{if isset($quest.itemreqs)}
		{foreach from=$quest.itemreqs key=i item=item}
			<tr>
				...
				<td>
					<span class="q{$item.quality}"><a href="/item-{$item.entry}.html">{$item.name}</a></span>{if $item.count>1} ({$item.count}){/if}
				</td>
			</tr>
		{/foreach}
	{/if}
	...
	{if isset($quest.objectreqs)}
		{foreach from=$quest.objectreqs key=i item=object}
			<tr>
				<th><ul><li><var>&nbsp;</var></li></ul></th>
				<td>
					<a href="/object-{$object.entry}.html">
...
{strip}
	{if isset($quest.factionreq)}
			<tr>
				<th><ul><li><var>&nbsp;</var></li></ul></th>
				<td>
					<a href="/faction-{$quest.factionreq.entry}.html">{$quest.factionreq.name}</a> ({$quest.factionreq.value})
				</td>
			</tr>
	{/if}
{/strip}
...
{if isset($quest.itemreqs)}
	<script type="text/javascript">
	{foreach from=$quest.itemreqs key=i item=item}
		getElement('iconlist-icon{$i}').appendChild(g_items.createIcon({$item.entry}, 0, {$item.count}));
	{/foreach}
	</script>
{/if}
{/if}
...
<table class="iconlist">
	<tr>
		...
		<td><span class="q1"><a href="/item-{$quest.SrcItem.entry}.html">{$quest.SrcItem.name}</a></span></td>
	</tr>
</table>
<script type="text/javascript">getElement('iconlist-icon-src').appendChild(g_items.createIcon({$quest.SrcItem.entry}, 0, {$quest.SrcItem.count}));</script>
{/if}
...
<table class="icontab">
	<tr>
		...
		<td><span class="q1"><a href="/spell-{$quest.SrcSpell.entry}.html">{$quest.SrcSpell.name}</a></span></td>
	</tr>
</table>
<script type="text/javascript">getElement('icontab-icon-spl').appendChild(g_spells.createIcon({$quest.SrcSpell.entry}, 0, 0));</script>
{/if}
...
{section name=j loop=$quest.itemchoices}
								...
								<td>
									<span class="q{$quest.itemchoices[j].quality}">
										<a href="/item-{$quest.itemchoices[j].entry}.html">
...
{/section}
						...
						<script type="text/javascript">
						{section name=j loop=$quest.itemchoices}
							getElement('icontab-icon{$smarty.section.j.index}').appendChild(g_items.createIcon({$quest.itemchoices[j].entry}, 1, {$quest.itemchoices[j].count}));
						{/section}
						</script>
{/if}
...
{section name=j loop=$quest.itemrewards}
								...
								<td>
									<span class="q{$quest.itemrewards[j].quality}">
										<a href="/item-{$quest.itemrewards[j].entry}.html">
...
{/section}
						...
						<script type="text/javascript">
						{section name=j loop=$quest.itemrewards}
							getElement('icontab-icon{$smarty.section.j.index+4}').appendChild(g_items.createIcon({$quest.itemrewards[j].entry}, 1, {$quest.itemrewards[j].count}));
						{/section}
						</script>
{/if}
...
{/if}
						...
						<tr>
						<th id="icontab-icon20"></th><td><a href="/spell-{$quest.spellreward.realentry}.html">{$quest.spellreward.name}</a></td>
						<th></th><td></td>
						</tr>
						</table>{/strip}

						<script type="text/javascript">
							getElement('icontab-icon20').appendChild(g_spells.createIcon({$quest.spellreward.entry}, 1, 0));
						</script>
{/if}
...
{section name=j loop=$quest.reprewards}
						<li><div>{$quest.reprewards[j].value} {#reputationwith#} <a href="/faction-{$quest.reprewards[j].entry}.html">{$quest.reprewards[j].name}</a></div></li>
{/section}
{/if}						
...
{/if}
					{/strip}</ul>
{/if}
<iframe allowfullscreen="true" style="overflow:hidden" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" width="100%" height="500px" src="/tts/index.html"></iframe>
				<h2>{#Related#}</h2>
