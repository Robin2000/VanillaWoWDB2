...
function Mapper(d, e) {
	...
	this.parent.appendChild(this.span = createElem("span"));
	c = this.span.style;
	c.display = "block";
	c.position = "relative";
	ns(this.span);
	...
	if (this.zoneLink) {
		...
		var b = createElem("a");
		b.href = "/zones.html";
		append(b, createText(f.firstChild.nodeValue));
		delElem(f.firstChild);
		append(f, b);
		c.style.display = "none";
		c.style.position = "absolute";
	...
	this.pinBag = c = createElem("div");
	append(this.span, this.pinBag);
	if (d.coords != null) {
...
function ma_initShowOnMap() {
	var e = getElement("lenrlkn4");
	var a = createElem("select");
	var c = createElem("option");
	c.value = "";
	c.style.color = "#bbbbbb";
	append(c, createText(LANG.showonmap));
	append(a, c);
	if (showOnMap.qg_alliance || showOnMap.qg_horde) {
		var b = createElem("optgroup", {
			label: LANG.som_questgivers
		});
		if (showOnMap.qg_alliance) {
			append(b, createElem("option", {
				value: "qg_alliance",
