...
Mapper.prototype = {
	...
	updateMap: function (c) {
		...
		if (this.zone == "0") {
			...
			if (this.zone==209||this.zone==1581||this.zone==2159||this.zone==1583||this.zone==15830||this.zone==3429) {
				d=1;/*防图片缓存*/
			}
			var ext = ".jpg)";
			if(this.zone == 2057) {
			...
			this.span.style.background = "url(images/maps/"+((g_locale.id==4)?'zhcn/':'enus/') + Mapper.sizes[this.zoom][2] + "/" + this.zone + (d ? "-" + d: "") + ext;
