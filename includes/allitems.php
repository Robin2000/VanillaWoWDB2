...
function render_item_tooltip(&$Row)
{
	...
	if($Row['maxcount']>1)
 	...
 	if($Row['start_quest'])
		$x .= '<br /><a class="q1" href="/quest-'.$Row['start_quest'].'.html">'.LOCALE_START_QUEST.'</a>';
	...
	if($Row['description'])
	{
		if($Row['spelltrigger_2']==6)
			$x .= '<span class="q2 ">'.LOCALE_GBONUS_USE.' <a href="/spell-'.$Row['spellid_2'].'.html">'.localizedName($Row, 'description').'</a></span>';
		else
	...
	if($row)
	{
		...
		for($i=1;$i<=10;$i++)
		{
			if($row['item'.$i] >0)
			{
				...
				$x_tmp .= '<span><a href="/item-'.$row['item'.$i].'.html">'.$name.'</a></span><br />';
			}
		}
		$x .= '<br /><span class="q"><a href="/itemset-'.$row['itemsetID'].'.html" class="q">'.$row['name_loc'.$_SESSION['locale']].'</a> (0/'.$num.')</span><br />';
		// Если требуется скилл
		if($row['skillID'])
		{
			$name = $DB->selectCell('SELECT name_loc'.$_SESSION['locale'].' FROM ?_skill WHERE skillID=?d LIMIT 1', $row['skillID']);
			$x .= LOCALE_REQUIRES.' <a href="/spells-11.'.$row['skillID'].'.html" class="q1">'.$name.'</a>';
			if($row['skilllevel'])
				$x .= ' ('.$row['skilllevel'].')';
			$x .= '<br />';
		}
