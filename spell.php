	...
	if($row)
	{
			...
			if($row['effect'.$j.'id'] > 0)
			{
				...
				if($row['effect'.$j.'MiscValue'])
				{
					switch ($row['effect'.$j.'id'])
					{
						...
						{
							$spell['effect'][$i]['name'] .= ' (<a href="/npc-'.$row['effect'.$j.'MiscValue'].'.html">'.$row['effect'.$j.'MiscValue'].'</a>)';
							break;
						}
						default:
						{
				...
				if($row['effect'.$j.'Aura'] > 0 && IsSet($spell_aura_names[$row['effect'.$j.'Aura']]))
				{
					...
					switch ($row['effect'.$j.'Aura'])
					{
						...
						case 56: // "Transform"
						{
							$spell['effect'][$i]['name'] .= ': '.$spell_aura_names[$row['effect'.$j.'Aura']].' (<a href="/npc-'.$row['effect'.$j.'MiscValue'].'.html">'.$row['effect'.$j.'MiscValue'].'</a>)';
							break;
						}
						default:
						{
		...
		if($trainers)
		{
			$taughtbytrainers = $DB->select('
				SELECT ?#, c.entry, A as faction_A,H as faction_H
		...
		if($taughtbyspells)
		{
			...
			$taughtbypets = $DB->select('
				...
				{ LEFT JOIN (locales_creature l) ON c.entry = l.entry AND ? }
				WHERE
					c.entry IN (SELECT entry FROM petcreateinfo_spell WHERE (spell_id1 IN (?a)) OR (spell_id2 IN (?a)) OR (spell_id3 IN (?a)) OR (spell_id4 IN (?a)))
					AND factiontemplateID=faction
				',
				$npc_cols[0],
				($_SESSION['locale']>0)? $_SESSION['locale']: DBSIMPLE_SKIP,
		...
		$usedbyitemset = $DB->select('
			SELECT *
			FROM ?_itemset
			WHERE spell_id1 = ?d OR spell_id2 = ?d OR spell_id3 = ?d OR spell_id4 = ?d OR spell_id5 = ?d OR spell_id6 = ?d OR spell_id7 = ?d OR spell_id8 = ?d
			',
